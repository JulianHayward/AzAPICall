...
| getGroupMembersCount          | `switch`    | beta endpoint (users, groups, servicePrincipals) '*count is not currently supported*' may be returned, however a retry may be successfull                                                                                |          |
...
| getMgAscSecureScore           | `switch`    | endpoint 'providers/Microsoft.ResourceGraph/resources' may return 'BadRequest' however a retry may be successfull - this parameter could be generalized for ARG queries                                                                                 |          |
