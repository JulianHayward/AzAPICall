...
| asynchronousAzureOperationNotWaitToFinish | switch | Don't wait to finish or reaching the max retries. |  |
| asynchronousAzureOperationMaxRetries | int | How ofter should the API be called to check the status of the asynchronous azure operation before exiting the function. If the status didn't reach a finish state (`Succeeded`, `Failed`, `Canceled`) you can save the result and check it later. | default is `10` |
| asynchronousAzureOperationRetryAfterSeconds | double | If the API is responding with a `Re-Try`, then this will be used. If not, you can use this parameter to define a static retry. Otherwise, if you don't use this parameter and the API won't responde with a `Re-Try` and random re-try will be set between `10` and `60` seconds. |  |
