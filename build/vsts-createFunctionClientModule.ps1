...
$parentModule = 'AzApiCall'
if (-not $ModuleName) { $ModuleName = 'AzApiCall.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\AzApiCall'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\AzApiCall.psd1"
$prereqHash = @{
